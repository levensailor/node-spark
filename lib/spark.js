...
function Spark() {
  ...
  this.init = function(options) {
    ...
    if(options && options.hasOwnProperty('swagger') && typeof options.swagger === 'string') {
      swaggerDef = options.swagger;
      if(!/^https?:/.test(swaggerDef) && !path.isAbsolute(swaggerDef)) {
        // normalize path as relative to folder containing node_modules
        swaggerDef = path.normalize(path.join(__dirname, '/../../../', swaggerDef));
      }
    } else {
      swaggerDef = path.normalize(path.join(__dirname, _swaggerDef));
    }

    var scheme = 'apiKey';
    var auth = new SwaggerClient.ApiKeyAuthorization('Authorization', 'Bearer ' + token, 'header');

    // load spark swagger api spec
    var apiURI = when.promise((resolve, reject) => {
      // if swaggerDef is url
      if(/^https?:/.test(swaggerDef)) {
        debug('Loading Spark API spec from %s', swaggerDef);

        // read swagger file from url
        request.get(swaggerDef, function(err, res) {
          if(err) {
            reject(new Error('can not access url at ' + swaggerDef));
          } else {
      ...
      // else, swaggerDef is file
      else {
        debug('Loading Spark API spec from %s', swaggerDef);

        // read file
        fs.readFile(swaggerDef, 'utf8', function(err, data) {
          if(err) {
            reject(new Error('can not read file ' + swaggerDef));
          } else {
