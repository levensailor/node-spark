...
function Spark() {
  ...
  var debug = function(msg) {
    ...
    if(process.env.hasOwnProperty('DEBUG') && process.env.DEBUG.toLowerCase().includes('spark')) {
      ...
      console.log(util.format('%s %s %s', chalk.bold('spark'), msg, chalk.cyan('+' + offset + 'ms')));
    }
  };

  // get full path for spark swagger definition file
  var getDefPath = function(defPath) {
    // if path passed...
    if(defPath) {
  ...
  this.getSpec = function(defPath, callback) {
    var args = Array.prototype.slice.call(arguments);
    defPath = args.length > 0 && typeof args[0] === 'string' ? args.shift() : getDefPath();
    ...
    // intercept promise and process optional callback
    .then(spec => {
      if(callback) {
  ...
  this.init = function(options, callback) {
    var args = Array.prototype.slice.call(arguments);
    options = args.length > 0 && typeof args[0] === 'object' ? args.shift() : {};
    callback = args.length > 0 && typeof args[0] === 'function' ? args.shift() : null;
    ...
    if(options && options.hasOwnProperty('swagger') && typeof options.swagger === 'string') {
      spark.def = getDefPath(options.swagger);
    } else {
      spark.def = getDefPath();
    }
    ...
    if(spark.token) {
      ...
      return spark.getSpec(spark.def)
        ...
        .then(spec => {
          ...
          // if callback, create callback friendly client
          if(callback) {
            return when.promise((resolve, reject) => {
              var client = new SwaggerClient({
                ...
                'success': function() {
                  ...
                  else reject(new Error('error initializng client'));
                }
              });
            });
          }

          // else, create promise friendly client
          else {
